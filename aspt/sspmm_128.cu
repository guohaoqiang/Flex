...
void ready2(int argc, char **argv){
    ...
    while(std::getline(ss2,word,',')){
        cols.push_back(std::stoi(word));        
    }
    
    if (data_name == "amazon.csv" || data_name=="reddit.csv"){
    ...
    memcpy(csr_v, rowPtr.data(), sizeof(int)*(nr+1));
    memcpy(csr_e0, cols.data(), sizeof(int)*ne);
    memcpy(csr_ev0, vals.data(), sizeof(FTYPE)*ne);
...
//        fprintf(stdout, "%f,%f,", tot_ms,(double)ne*2*sc/tot_ms/1000000);
        if (false){
            ...
            //fprintf(fpo, "%f,", (double)tot_ms*1000/ITER);
            fprintf(fpo2, "%f", (double)ptot_ms/tot_ms);
        }else{
            printf("GFLOPS: %f\n", (double)ITER*(double)ne*2*sc/tot_ms/1000000);
            //fprintf(fpo, "%f,", (double)tot_ms*1000/ITER);
            printf("t_pre/t_exe: %f\n", (double)ptot_ms/tot_ms);
        }
...
//      fprintf(stdout, "num_diff : %d\n", num_diff);
        if (false){
            ...
            printf("errs: %f", (double)num_diff/(nr*sc)*100);
        }
